...
> `127.0.0.1:8765` 可由 Camera Activity Notifier 等本机服务使用。HaloLink v0.1.6
> 不会占用或修改该服务。
...
```bash
adb install -r apk/HaloLink-v0.1.6-debug.apk
```
...
- `/api/state` WebSocket 断开时每 1.5 秒启用的 HTTP polling 备用通道
...
- mDNS MulticastLock 只在发现 Bridge 期间持有，找到或停止发现后立即释放。
- Ready 和 Completed 为静态圆环，不再按屏幕刷新率持续重绘；每两分钟只重绘一次，
  用于轻微移动像素、降低 OLED 烧屏风险。
- Thinking、Working、Responding、Listening 和 Error 保留原来的动态效果。
- Ready 或 Completed 保持 30 秒后，将当前 App 窗口亮度限制在 12% 以内；出现新状态时
  立即恢复系统亮度。
...
工作流会编译 v0.1.6 debug APK、生成 SHA-256，并上传构建产物。
